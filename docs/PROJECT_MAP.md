...
- `DashisDashboardDetail.swift`：无重复页标题的 Dashboard 摘要列表与单 provider detail 路由；OpenRouter 首次 `Set up account` 会直接导航到详情；页标题为 32 pt serif，详情外边距为 horizontal 30 / top 26 / bottom 30，纵向 spacing 为 14，provider 内容最大宽度 900，外层最大宽度 1180。
- `DashisDashboardComponents.swift`：Dashboard 扁平 provider 摘要、条件式风险限定词、Codex Analytics 风格的两列 quota/balance/metric 主卡片、折叠的额外指标/metadata、非卡片式 warning/failure，以及 Codex/Claude/Google/OpenRouter 原生控件；OpenRouter account 详情含独立的 metadata-only recent-call disclosure。
...
- `DashisProviderStore.swift`：UI state、session-only 输入/credential、显式动作、Clear generation guard 和 snapshot 投影；OpenRouter 默认使用账户级 management-key 模式，单-key OAuth 为可选模式，recent calls 使用不污染账户 snapshot 的独立 state；Debug `--visual-qa` 可注入纯合成 Codex snapshot 进行视觉验收，不读取真实账户或凭据。
